...
#define	SUPPORT_FAKE_PS2_CMD		TRUE
...
void SendCmdtoMouse(BYTE PortNum)
{
...
#if SUPPORT_FAKE_PS2_CMD

		Aux_Data_To_Host(0xFA);
#else
    ...
    else                    // Other commands
    {
    	...
    	if (KBHIData == 0xFF) 
    	{
			SoftMouseByteCount = 2;
			SoftMouseDataDir = 0;
			SoftMouseData[0] = 0xAA;
			SoftMouseData[1] = 0x00;
    	}
    	else if (KBHIData == 0xE9)  
    	{
			SoftMouseByteCount = 3;
			SoftMouseDataDir = 0;
			SoftMouseData[0] = 0x00;
			SoftMouseData[1] = 0x00;
			SoftMouseData[2] = 0x64;
    	}
    	else if (KBHIData == 0xF2)  
    	{
			SoftMouseByteCount = 1;
			SoftMouseDataDir = 0;
			SoftMouseData[0] = 0x00;
    	}
#else    
...
void ReSendPS2PendingData(void)
{
    ...
    //-------------------------------------------------------------------------
    /* Send Soft Mouse Packet */
    //-------------------------------------------------------------------------
    if (SoftMouseByteCount > 0)
    {
        ...
        ITempB03 = SoftMouseData[SoftMouseDataDir];
        SoftMouseDataDir++;
        SoftMouseByteCount--;
        //OEM_RamDebug(ITempB03);
        SendFromAux(ITempB03);
    }
    //-------------------------------------------------------------------------
}
